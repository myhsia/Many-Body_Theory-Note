...
\]
then we start from
$\identity \in co Basis set$. Eq of $\identity$, $u^{\alpha_0} = \identity$:
\[
  0 = 0 + \sum ...
\]
...
\begin{enumext}
  ...
  \[
    \sum_n \ketbra|\varphi_n><\varphi_n| = \identity,
  \]
  While in the coherent state
  \begin{equation}
    \identity = \frac1\pi \int\ketbra|\alpha><\alpha| \d \Re\alpha \d \Im\alpha
  \end{equation}
...
and we can insert the identity $\identity$
...
while the identity
\begin{equation}
  \identity = \int \d\mu[\bm n] \ketbra|\bm n><\bm n|
\end{equation}
where
\begin{equation}
