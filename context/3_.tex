...
Then you can define Poisson bracket $\{\bar x, p\} = \identity \Rightarrow [\hat x, \hat p] = \identity$.
...
\begin{framed}
  IDENTITY IS VERY IMPORTANT! $\identity$
  \[\identity = \int \d x\ketbra|x><x|\]
\end{framed}
Then we have
\[
...
Identity
\[\identity = \frac1{N!}\sum_{n_1, ..., n_i} \ketbra|n_1,n_2,n_3,...><...| = \int \d x\ketbra|x><x| = \int \d x\psi^\dagger(x)\cancel{\ketbra|0><0|}\psi(x) = \int \d x_1 \ldots \d x_n \ketbra|x_1,...,x_i><...|\]
